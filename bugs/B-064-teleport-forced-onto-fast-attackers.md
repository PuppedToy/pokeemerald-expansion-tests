...
The rater would never pick it — `chooseMoveset` with no forced move returns
`MOONBLAST / PSYCHIC / AURA_SPHERE / FRENZY_PLANT`. Forcing only `MOVE_TELEPORT` reproduces the run's
set exactly: `TELEPORT / MOONBLAST / PSYCHIC / AURA_SPHERE`.
...
2. `planMemberRoleMove` in `randomizer/modules/archetypeRefine.js` returned the **first** move of the
   role's set in insertion order that passed the B-030 TM-bag gate. Wally Lilycove does not yet hold
   the U-turn / Volt Switch / Flip Turn TMs, so the loop fell through to Teleport — a level-1 move of
   the Ralts line, always reachable.
...
- `randomizer/modules/featureDetectors.js` — Teleport leaves `PIVOT_MOVES` for `SLOW_PIVOT_MOVES`, and
  ...
  actually closes the shipped case — Gardevoir learns the pivot TMs, so it is a legitimate pivot species,
  and only the delivery gate stops Teleport reaching it when Wally holds none of those TMs.
...
Verified against the run's own bundle: Teleport 1.93 → 1.00 for that Gardevoir, and the pivot role move
offered with Wally Lilycove's (empty) pivot-TM bag goes from `MOVE_TELEPORT` to none. A slow bulky
