...
// T-149 — a trainer's level is DERIVED from its segment's boss cap (caps.c is the SSOT), never hard-coded.
// CAP.<name> is the cap FLAG a trainer tracks; getTrainersData resolves it to a concrete level via the
// injected `capLevels` map (baked into base-data.json for the browser, parsed from caps.c in node). Change
// a cap level in caps.c and every trainer tuned to it follows automatically. Trainers that share a cap
// share a level — that is the point (previously they were independent magic numbers that silently desynced).
const CAP = {
    RIVAL_ROUTE103:     'FLAG_DEFEATED_RIVAL_ROUTE103',
    AQUA_WOODS:         'FLAG_DEFEATED_AQUA_WOODS',
    ROXANNE:            'FLAG_BADGE01_GET',
    DEVON_RUSTURF:      'FLAG_RECOVERED_DEVON_GOODS',
    RIVAL_RUSTBORO:     'FLAG_DEFEATED_RIVAL_RUSTBORO',
    BRAWLY:             'FLAG_BADGE02_GET',
    STEVEN_GRANITE:     'FLAG_DELIVERED_STEVEN_LETTER',
    DEVON_MUSEUM:       'FLAG_DELIVERED_DEVON_GOODS',
    RIVAL_ROUTE110:     'FLAG_ROUTE110_RIVAL_DEFEATED',
    WALLY_MAUVILLE:     'FLAG_DEFEATED_WALLY_MAUVILLE',
    WATTSON:            'FLAG_BADGE03_GET',
    TABITHA_CHIMNEY:    'FLAG_DEFEATED_TABITHA_MT_CHIMNEY',
    FLANNERY:           'FLAG_BADGE04_GET',
    NORMAN:             'FLAG_BADGE05_GET',
    SHELLY_WEATHER:     'FLAG_DEFEATED_SHELLY_WEATHER_INST',
    RIVAL_ROUTE119:     'FLAG_ROUTE119_RIVAL_DEFEATED',
    WINONA:             'FLAG_BADGE06_GET',
    WALLY_LILYCOVE:     'FLAG_MET_RIVAL_LILYCOVE',
    MAXIE_MAGMA:        'FLAG_GROUDON_AWAKENED_MAGMA_HIDEOUT',
    MATT_AQUA:          'FLAG_TEAM_AQUA_ESCAPED_IN_SUBMARINE',
    TATE_LIZA:          'FLAG_BADGE07_GET',
    MAGMA_SPACE_CENTER: 'FLAG_DEFEATED_MAGMA_SPACE_CENTER',
    KYOGRE_SEAFLOOR:    'FLAG_KYOGRE_ESCAPED_SEAFLOOR_CAVERN',
    JUAN:               'FLAG_BADGE08_GET',
    WALLY_VICTORY_ROAD: 'FLAG_DEFEATED_WALLY_VICTORY_ROAD',
    RIVAL_EVERGRANDE:   'FLAG_DEFEATED_EVERGRANDE_RIVAL',
    E4_SIDNEY:          'FLAG_FIRST_DEFEATED_ELITE_4_SIDNEY',
    E4_PHOEBE:          'FLAG_FIRST_DEFEATED_ELITE_4_PHOEBE',
    E4_GLACIA:          'FLAG_FIRST_DEFEATED_ELITE_4_GLACIA',
    E4_DRAKE:           'FLAG_FIRST_DEFEATED_ELITE_4_DRAKE',
    CHAMPION:           'FLAG_IS_CHAMPION',
};

// T-149 — node-only fallback: read caps.c directly when a direct caller (unit test / CLI) does not inject
// capLevels. The pipeline (browser + node) always injects it via the pokédex artifact, so this is only hit
// by tests that call getTrainersData() directly. In the browser bundle `fs` is a stub and this never runs.
let _capLevelsFallback = null;
function loadCapLevelsFromCapsC() {
    if (!_capLevelsFallback) {
        const fs = require('fs');
        _capLevelsFallback = capLevelMap(fs.readFileSync(path.resolve(__dirname, '..', 'src', 'caps.c'), 'utf-8'));
    }
    return _capLevelsFallback;
}
...
// T-162 — Surf becomes available after beating Norman (by the 4th rival encounter, same gate as the
// Good Rod). From here on the rival can also field the SURF encounters of every zone reached so far.
// (Jagged Pass was entirely missing from the rival pools — added here across its methods.)
const rivalSurfEncounters = [
    ...getWildEncountersFromMap('MAP_ROUTE102', ['surf']),
    ...getWildEncountersFromMap('MAP_ROUTE103', ['surf']),
    ...getWildEncountersFromMap('MAP_ROUTE104', ['surf']),
    ...getWildEncountersFromMap('MAP_PETALBURG_WOODS', ['surf']),
    ...getWildEncountersFromMap('MAP_ROUTE115', ['surf']),
    ...getWildEncountersFromMap('MAP_ROUTE116', ['surf']),
    ...getWildEncountersFromMap('MAP_ROUTE106', ['surf']),
    ...getWildEncountersFromMap('MAP_ROUTE109', ['surf']),
    ...getWildEncountersFromMap('MAP_ROUTE110', ['surf']),
    ...getWildEncountersFromMap('MAP_ROUTE117', ['surf']),
    ...getWildEncountersFromMap('MAP_ROUTE118', ['surf']),
    ...getWildEncountersFromMap('MAP_ROUTE111', ['surf']),
    ...getWildEncountersFromMap('MAP_ROUTE112', ['surf']),
    ...getWildEncountersFromMap('MAP_JAGGED_PASS', ['surf']),
    ...getWildEncountersFromMap('MAP_ROUTE113', ['surf']),
    ...getWildEncountersFromMap('MAP_ROUTE114', ['surf']),
    ...getWildEncountersFromMap('MAP_ROUTE119', ['surf']),
];
...
const rival119Encounters = [
    ...
    ...getWildEncountersFromMap('MAP_JAGGED_PASS', ['land', 'old', 'good']),
...
const rivalSuperRodEncounters = [
    ...
    ...getWildEncountersFromMap('MAP_JAGGED_PASS', ['super']),
...
const rivalEvergrandeCityEncounters = [
    ...
    ...getWildEncountersFromMap('MAP_ROUTE120', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_SCORCHED_SLAB', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_ROUTE121', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_ROUTE122', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_MT_PYRE_EXTERIOR', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_ROUTE123', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_ROUTE124', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_ROUTE125', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_SHOAL_CAVE_LOW_TIDE_ENTRANCE_ROOM', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_SHOAL_CAVE_HIGH_TIDE_ENTRANCE_ROOM', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_ROUTE127', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_ROUTE126', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_ROUTE128', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_ROUTE129', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_ROUTE131', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_PACIFIDLOG_TOWN', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_ROUTE132', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('EVER_GRANDE_CITY', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
    ...getWildEncountersFromMap('MAP_VICTORY_ROAD_B1F', ['land', 'old', 'good', 'surf', 'super', 'underwater']),
];
...
function getTrainersData(itemAssignments, tmList, config = {}, capLevels = null) {
...
const trainersData = [
    ...
    // T-149 — TRAINER_MAXIE_MT_CHIMNEY removed: his battle was dropped in T-148 (he's gone). The
    // opponents.h constant and the hand-maintained sNonGymBosses[] C list keep referencing it harmlessly
    // (it is never battled). MAXIE_MEGA cross-appearance continuity is anchored on MAXIE_MAGMA_HIDEOUT.
    // Jagged Pass
    {
        id: 'TRAINER_ERIC',
    ...
    // T-149 — resolve each trainer's CAP.<name> level reference (a cap flag) to a concrete level from
    // the caps SSOT. Idempotent: once resolved to a number, later calls skip it (capLevels is constant
    // for the run). `copy:` trainers carry no level of their own — they inherit the (resolved) source's
    // level at copy time in the writer.
    const caps = capLevels || loadCapLevelsFromCapsC();
    for (const trainer of trainersData) {
        if (typeof trainer.level === 'string') {
            const resolved = caps[trainer.level];
            if (resolved === undefined) {
                throw new Error(`getTrainersData: no cap level for ${trainer.level} (trainer ${trainer.id})`);
            }
            trainer.level = resolved;
        }
    }
