...
Acceptance criteria:
- [ ] Inside the gauntlet the start menu has no PC entry; in the lobby, Hall of Fame and the rest of the
      world it does.
- [ ] Inside the gauntlet the summary screen offers no Relearn prompt when `leagueMoveRelearnAllowed` is
      off, and does offer it when the toggle is on.
- [ ] TM teaching is unaffected everywhere.
- [ ] Both restrictions are gone after a loss (player is in the lobby) and after the Champion win (player
      is in the Hall of Fame).
- [ ] `cd randomizer && npm test` green (+ backend suite), changelog line added.
